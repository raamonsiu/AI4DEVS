...
1. Their **bucket** matches exactly. The bucket is a deterministic tag composed
   ...
   with different form options will NEVER share a cache entry even if their
   descriptions are similar — the rendered prompt is different, so the
   estimation should be different too.
...
never returns a hit — useful for calibrating the threshold against real traffic
...
class EstimationSemanticCache:
    ...
    def _ensure_index(self) -> None:
        """Create the search index if it isn't there. Safe to call repeatedly:
        an already-exists error is the normal case and is swallowed."""
        try:
    ...
    def lookup(self, request: EstimationRequest, prompt_version: str) -> EstimationResult | None:
        """Return a cached result for a semantically equivalent request, or None.

        Fails open: this cache is an optimisation, so an embedding error, a
        dropped index (FLUSHALL, eviction, a Redis restart) or any other backend
        problem degrades to a cache miss and the pipeline calls the LLM as
        normal. Letting it raise would turn a cache hiccup into a 502 on every
        request, which is exactly the outage the cache is supposed to soften.
        """
        try:
            return self._lookup(request, prompt_version)
            ...
            # The index can disappear under us (FLUSHALL / eviction). Try to
            # rebuild it so subsequent requests can use the cache again.
            self._ensure_index()
            return None

    def _lookup(self, request: EstimationRequest, prompt_version: str) -> EstimationResult | None:
