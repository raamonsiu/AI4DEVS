...
Policy: all three raise ``InputGuardrailViolation`` (exception, never
fix-and-retry). Reasons live in the ``reason`` attribute so the HTTP layer can
pick a status code and the client can render an appropriate message.

Off-topic input (a recipe, a trivia question) is deliberately NOT handled here:
it is an *output*-side concern. The prompt instructs the model to answer with an
``Out of scope:`` summary and ``confidence_pct`` below the threshold, and
``guardrails/output.py`` normalises that into a clean user-facing message. That
costs no extra LLM call and returns a helpful 200 instead of a blunt 400.
"""
...
def _check_moderation(description: str, openai_client: Any) -> None:
    try:
        ...
        log.warning("moderation_call_failed", error_type=type(exc).__name__, error=str(exc))
        return
